...
const updateGlobalState = (updates: { room?: Room | null; playerId?: string | null; messages?: any[]; isInitialLoading?: boolean }) => {
  ...
  if (updates.isInitialLoading !== undefined) globalIsInitialLoading = updates.isInitialLoading;
...
export function useGameState() {
  ...
  const [isInitialLoading, setIsInitialLoading] = useState(globalIsInitialLoading);
  const [isConnected, setIsConnected] = useState(wsClient?.isConnected() || false);
  ...
  useEffect(() => {
    ...
    const unsubscribers = [
      ...
      wsClient.on(WSEvent.ROOM_CREATED, (payload) => {
        updateGlobalState({ room: payload.room, playerId: payload.playerId, isInitialLoading: false });
      ...
      wsClient.on(WSEvent.PLAYER_JOINED, (payload) => {
        ...
        const exists = globalRoom.players.some((p: Player) => p.id === payload.player.id);
        if (exists) return;
      ...
      wsClient.on(WSEvent.PLAYER_LEFT, (payload) => {
        if (!globalRoom) return;
        const players = globalRoom.players.filter((p: Player) => p.id !== payload.playerId);
        updateGlobalState({ room: { ...globalRoom, players } });
      }),
      ...
      wsClient.on(WSEvent.ROLE_ASSIGNED, (payload) => {
        if (!globalRoom) return;
        const players = globalRoom.players.map((p: Player) => 
          p.id === globalPlayerId ? { ...p, role: payload.role, word: payload.word } : p
        );
        updateGlobalState({ room: { ...globalRoom, players } });
      }),
          ...
          room: {
            ...
            game: {
              ...
              turnOrder: {
                ...
                currentIndex: globalRoom.game.turnOrder.playerIds.indexOf(payload.playerId)
              }
            }
          }
        });
      ...
      wsClient.on(WSEvent.VOTE_RESULT, (payload) => {
          if (!globalRoom) return;
          const players = globalRoom.players.map((p: Player) => 
            p.id === payload.eliminatedId ? { ...p, isAlive: false, role: payload.role } : p
          );
          updateGlobalState({ room: { ...globalRoom, players } });
      }),
      ...
      wsClient.on(WSEvent.ERROR, (payload) => {
        ...
        if (payload.code === ErrorCode.ROOM_NOT_FOUND || payload.message?.toLowerCase().includes('not found')) {
          clearSession();
        }
    ...
    if (!globalRoom) {
      ...
      if (savedRoomId && savedPlayerId) {
        wsClient.send(WSEvent.RECONNECT, { roomId: savedRoomId, playerId: savedPlayerId });
      } else {
        updateGlobalState({ isInitialLoading: false });
      }
    } else {
